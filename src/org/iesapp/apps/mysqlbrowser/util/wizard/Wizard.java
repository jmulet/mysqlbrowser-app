...
public class Wizard extends WindowAdapter implements PropertyChangeListener {
    ...
    public Wizard(Dialog owner) {
        ...
        wizardDialog = new WizardUI(null);    
        wizardDialog.setWizard(this);
        initComponents();
    }
    ...
    public Wizard(Frame owner) {
        wizardModel = new WizardModel();
        wizardDialog = new WizardUI(owner);    
        wizardDialog.setWizard(this);
        initComponents();
    }
    ...
    public int showModalDialog() {
        ...
        if(this.getModel().getTitleStepMap().isEmpty())
        {
            wizardDialog.setStepTitlesVisible(false);
        }
    ...
    public void registerWizardPanel(Object id, String title, WizardPanelDescriptor panel) {
        ...
        wizardModel.registerPanel(id, title, panel);
        wizardDialog.getStepsPane().add((JLabel) wizardModel.getTitleStepMap().get(id));       
    }  
    ...
    public void setCurrentPanel(Object id) {
        ...
        //System.out.println("Set current panel id is "+id);
        ...
        for(Object value: wizardModel.getTitleStepMap().values())
        {
            ((JLabel) value).setFont(new java.awt.Font("Tahoma", 0, 11));  
        }
         
        if(wizardModel.getTitleStepMap().containsKey(id))
        {
            wizardDialog.setStepTitle(((JLabel) wizardModel.getTitleStepMap().get(id)).getText());
            ((JLabel) wizardModel.getTitleStepMap().get(id)).setFont(new java.awt.Font("Tahoma", 1, 12));
        }
   ...
   /**
    ...
    public void close(int code) {
        returnCode = code;
        //Si estic en la darrera passa, he d'invocar el metode abouttohide
        WizardPanelDescriptor oldPanelDescriptor = wizardModel.getCurrentPanelDescriptor();
        if (oldPanelDescriptor != null) {
            oldPanelDescriptor.aboutToHidePanel();
        }
    ...
    private void initComponents() {
        ...
        cardPanel =  wizardDialog.getCardPane();
        cardPanel.setBorder(new EmptyBorder(new Insets(5, 10, 5, 10)));       

        cardLayout = new CardLayout(); 
        cardPanel.setLayout(cardLayout);
        
        backButton = new JButton(new ImageIcon("/org/iesapp/framework/util/wizard/backIcon.gif"));
        ...
        wizardDialog.getButtonPanel().setLayout(new BorderLayout());
        wizardDialog.getButtonPanel().add(separator, BorderLayout.NORTH);
        ...
        wizardDialog.getButtonPanel().add(buttonBox, java.awt.BorderLayout.EAST);
    ...
    private static Object getImage(String name) {
        ...
        try {
            Class c = Class.forName("org.iesapp.framework.util.wizard.Wizard");
    ...
    static {
        ...
        try { 
            ...
            PropertyResourceBundle resources = (PropertyResourceBundle)
                ResourceBundle.getBundle("org.iesapp.framework.util.wizard.wizard");
